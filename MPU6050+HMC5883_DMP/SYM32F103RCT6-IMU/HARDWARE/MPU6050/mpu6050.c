...
uint8_t mpu6050_write(uint8_t addr,uint8_t reg, uint8_t len, uint8_t *dat)
{
	uint8_t res = 0;
	#if MPU6050_IIC_TYPE
	res	= Software_Write_longByte(addr, reg, len, dat);
	if(res == 0)
		return MPU6050_EOK;
	else
		return MPU6050_EACK;
	#else
	res = Hardware_Write_longByte(addr, reg, len, dat);
	if(res == 0)
		return MPU6050_EOK;
	else
		return MPU6050_EACK;
...
uint8_t mpu6050_read(uint8_t addr, uint8_t reg, uint8_t len, uint8_t *dat)
{
	uint8_t res = 0;
	#if MPU6050_IIC_TYPE
  res = Software_Read_Byte(addr, reg, len, dat);
	if(res == 0)
		return MPU6050_EOK;
	else
		return MPU6050_EACK;
	#else
	res = Hardware_Read_Byte(addr, reg, len, dat);
	if(res == 0)
		return MPU6050_EOK;
	else
		return MPU6050_EACK;
