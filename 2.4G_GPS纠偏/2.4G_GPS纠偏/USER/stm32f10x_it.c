...
void USART1_IRQHandler(void)
{
  ...
  int 			  rx1DMA_CNT   =0;	  //记录本次接收到的数据个数
	...
	{		
		...
		rx1DMA_CNT = UART_RX1_SIZE - RX1_DMA_CH->CNDTR ; //读取本次DMA接收到数据个数
//		Buff_USART1_RX[rx1DMA_CNT]='\0';
		/* 判断是否数据头 且  上包数据已处理完 */
		if( custom_str_find((char*)Buff_USART1_RX, rtcm_Head ,rx1DMA_CNT) &&  Flag_DMA_RX1==0 )
		{
			if(flag_rxData )  												  //标志位未清除 (未收到尾再次收到头，数据不完整)
			{ 
				rx1Data_CNT = 0;     										  //清除计数(数据包总个数)
				flag_rxData=0;       										  //清除标志位		
			}	
			
			rx1Data_CNT = rx1DMA_CNT;  						   		//总数据个数记录
			memcpy(rxBuff, Buff_USART1_RX, rx1DMA_CNT);	//保存包含数据头的数据
			flag_rxData = 1;   													//已接收数据头标志位，置位后开始接收保存数据直到再次接收到数据头或尾
		...
		else if(flag_rxData)
		{
			...
			if((rx1Data_CNT+rx1DMA_CNT) > rxBuff_SIZE)																//数据越界
			{
				rx1Data_CNT = 0;
				flag_rxData = 0;
			}
			...
			{
				memcpy(rxBuff + rx1Data_CNT, Buff_USART1_RX, rx1DMA_CNT);	  //保存数据
				/* 是尾帧数据 */
				if(custom_str_find( (char*)Buff_USART1_RX , rtcm_End , rx1DMA_CNT) )
				{
					flag_rxData  = 0;  //数据接收完成标志位清除
					Flag_DMA_RX1 = 1;	 //整包数据接收完成标志位
				}	
				rx1Data_CNT += rx1DMA_CNT;  										            //总数据个数记录
			}	
		}
