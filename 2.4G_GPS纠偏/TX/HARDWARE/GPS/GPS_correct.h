#ifndef __GPS_CORRECT
#define __GPS_CORRECT
...
void nmeagga(char *data ,uint8_t buf[],uint16_t Len);//提取nmea数据中GGA字段
char decode_nmea(nmea_raw_t *raw);     //分割GGA字段各数据
int decode_nmeagga( nmea_raw_t *raw, char **val ,int n);//各字段数据格式处理转换


int RTCM_CRC(uint8_t* rtcm,uint16_t size);
int RTCM_Parse(uint8_t *data,  uint16_t data_len);

static int rtcm_crc_table[]=
{
